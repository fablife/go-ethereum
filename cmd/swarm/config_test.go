...
func TestFileOverrides(t *testing.T) {
	...
	if info.HiveParams.CallInterval != 6000000000 {
		t.Fatalf("Expected HiveParams CallInterval to be %d, got %d", 6000000000, info.HiveParams.CallInterval)
	}
...
func TestEnvVars(t *testing.T) {
	...
	envVars := []string{
		fmt.Sprintf("%s=%s", SwarmPortFlag.EnvVar, httpPort),
		fmt.Sprintf("%s=%s", SwarmNetworkIdFlag.EnvVar, "999"),
		fmt.Sprintf("%s=%s", CorsStringFlag.EnvVar, "*"),
		fmt.Sprintf("%s=%s", SwarmSyncEnabledFlag.EnvVar, "true"),
	}
